...
class LibraryAnalyser:
    ...
    def is_lib_call(self, operand):
        if is_hex(operand):
            ...
            plt_boundaries = [self.__plt_section.virtual_address, self.__plt_section.virtual_address + self.__plt_section.size]
            return operand >= plt_boundaries[0] and operand < plt_boundaries[1]
        else:
            #TODO
            pass
    ...
    def get_function_called(self, operand):
        operand = int(operand, 16)
        plt_offset = operand - self.__plt_section.virtual_address
    ...
    def get_used_syscalls(self, syscalls_set, functions):
        ...
        if len(functions) > 0:
            print_debug(f"Function {functions[0].name} is present in {functions[0].library_path} at address {hex(functions[0].address)}")
    ...
    def __get_got_rel_address(self, int_operand):
        ...
        for instr in self.__md.disasm(bytearray(self.__plt_section.content)[plt_offset:], int_operand):
        ...
        return int(instr_at_address.op_str.split()[-1][:-1], 16) + instr_next.address

    def __find_function_with_name(self, f_name):
        functions = []
        for lib in self.__used_libraries.values():
            if f_name in lib.callable_fun_addresses:
                to_add = LibFunction(name=f_name, library_path=lib.path, address=lib.callable_fun_addresses[f_name])
        ...
        if functions == []:
            ...
            sys.stderr.write(f"[WARNING] Multiple possible library functions were found for {f_name}: {functions}.\n All of them will be considered.\n")
    ...
    def __add_used_library(self, lib_path):
        ...
        if lib_name not in self.__used_libraries:
            print_verbose("[WARNING] A library path was added for a library that was not detected by `lief`.")

        lib_binary = lief.parse(lib_path)
        callable_fun_addresses = dict()
        for item in lib_binary.dynamic_symbols:
            callable_fun_addresses[item.name] = item.value
        self.__used_libraries[lib_name] = Library(path=lib_path, callable_fun_addresses=callable_fun_addresses)
    ...
    def __find_used_libraries(self):
        ...
        try:
            ...
            if not all(self.__used_libraries.values()):
                print_verbose("[ERROR] The `ldd` command didn't find all the libraries used.\nTrying to find the remaining libraries' path manually...")
    ...
    def __find_used_libraries_manually(self):
        lib_names = [name for name, lib in self.__used_libraries.items() if lib is None]
